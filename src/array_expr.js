...
/* ***************************************************
* Define the Array Expression (vectors, vector-valued expressions, tensors)
* *************************************************** */
...
export class array_expr extends expression {
    ...
    allStringEquivs() {
        ...
        function buildStringEquivs(indexList, leftStr) {
            if (typeof leftStr == "undefined" || leftStr == '') {
                leftStr = "(";
            }
            if (indexList.length > 0) {
                var workInputs = allInputsArrays[indexList[0]];
                for (var i in workInputs) {
                    buildStringEquivs(indexList.slice(1), 
                        leftStr + 
                        workInputs[i] +
                        indexList.length > 1 ? "," : "");
                }
    ...
    toTeX(showSelect) {
        ...
        for (var i in this.inputs) {
            ...
            if (typeof this.inputs[i] == 'undefined') {
                ...
                theStr += this.inputs[i].toTeX(showselect);
            }
        }
        theStr += '\\rangle';
    ...
    // Unclear what happens here if there are nested arrays
    // and whether the result is considered valid mathml
    toMathML() {
        var theStr = "<mrow><mo>(</mo><mtable>";
        for (var i in this.inputs) {
            theStr += "<mtd>";
            theStr += this.inputs[i].toMathML();
            theStr += "</mtd>";
        }
        theStr += "</mtable><mo>)</mo></mrow>";
        return(theStr);
    }
    ...
    // An evaluated array is still an array.
    evaluate(bindings) {
        var inputVal;
        var i;
        var retVal;
